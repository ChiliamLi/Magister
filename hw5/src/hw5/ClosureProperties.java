...
//All graph properties can be found in graph.java
    ...
    public boolean BipartiteMatching (Map adjList) {
       ...
       /* Map<String, String> colorMap = new HashMap<String, String>();
        for (List<String> matching: matchings) {
            if (!colorMap.containsKey(matching.get(0)) && (!colorMap.containsKey(matching.get(1)) || 
                    !colorMap.get(matching.get(1)).equals("red"))) {
                // if hashmap does not contain tutor and neighboring node DNE or is of opposite color
                colorMap.put(matching.get(0), "red"); // create new entry
                if (!colorMap.containsKey(matching.get(1))) {
                    colorMap.put(matching.get(1), "blue");
                }
